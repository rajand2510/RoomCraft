...
const AddToCartButton = () => {
  ...
  return (
    <button
      className="flex gap-1 px-5 w-2/12 py-1.5 mt-7 text-base font-bold text-justify text-white bg-black rounded-3xl"
...
const ProductDetails = ({ title, discription, price, gltfPath }) => {
  return (
    <section className="flex flex-col self-stretch mt-4 my-auto">
      ...
      <p className="mt-4 text-3xl font-bold text-justify text-black">
        Rs.{price}
      </p>
      <AddToCartButton />
...
const MainProduct = () => {
  ...
  return (
    <>
      <Navbar />
      <div
        className={`px-4 py-8    ${isMobile ? "mt-[15px]" : "mt-[150px]"
          } mb-20   bg-white rounded-3xl shadow-xl drop-shadow-2xl ${isMobile ? "max-w-full" : "max-w-[898px] mx-auto"
          }`}
      >
        <div className={`flex gap-5 ${isMobile ? "flex-col" : "flex-row"}`}>
          <div
            className={`flex flex-col w-full ${isMobile ? "-mb-6" : "w-1/2"}`}
          >
           <div
  ...
  style={{
    ...
    margin: "0 auto",   // Centering for better alignment
  }}
>
  <model-viewer
    src={gltfPath}
    ...
    style={{
      width: "100%",
      height: "100%",
      objectFit: "contain",  // Ensure the model fits within the container
    }}
  ></model-viewer>
</div>
